...
Write-Host ""
Write-Host "[4/5] Admin consent" -ForegroundColor Yellow
try {
...
Write-Host ""
Write-Host "[5/5] Client secret (valid $SecretYears years)" -ForegroundColor Yellow
...
if (Test-Path $envPath) {
    ...
    $existing = Get-Content $envPath | Where-Object { $_ -notmatch '^(M365_TENANT_ID|M365_CLIENT_ID|M365_CLIENT_SECRET)=' }
    ($existing + '' + $envLines) | Set-Content $envPath -Encoding UTF8
} else {
    $envLines | Set-Content $envPath -Encoding UTF8
}
Write-Host ""
Write-Host ".env updated. Restart the server:  npm start" -ForegroundColor Cyan
Write-Host ""
