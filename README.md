...
| Feature Geography | `/featuregeo` | `featuregeo.html` | `releaseplans.microsoft.com` (scraped + normalised) |
...
| `GET /api/featuregeo` | Feature availability by geography, normalised from the public release plans site (30 min server cache) | None | 60/min |
...
featuregeo.html                  Feature availability by geography (world map + rollout table)
...
static/
  ...
  featuregeo.css                 Feature Geography page styles (map, filter rail, rollout table)
  worldmap.js                    Generated world outline (Natural Earth 110m) used by the Feature Geography map
